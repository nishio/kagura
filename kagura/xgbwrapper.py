...
eta: 0.01[1], 0.4-1.0[2]
max_depth 9[1], 4-10[2], default 6
subsample [default=1] 0.4-1[2]
colsample_bytree [default=1] 0.4-1[2]
...
class XGBWrapper(object):
    ...
    def predict_proba(self, xs):
        ...
        N = xs.shape[0]
        xs = xgb.DMatrix(xs)
        ypred = self.bst.predict(xs, ntree_limit=self.bst.best_iteration)
        ypred = ypred.reshape(N, self.params['num_class'])
        return ypred

    def score(self, xs, ys):
        from sklearn.metrics import log_loss
        ypred = self.predict_proba(xs)
        return log_loss(ys, ypred)
